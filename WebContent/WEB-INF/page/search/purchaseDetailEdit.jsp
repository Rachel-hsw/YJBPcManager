...
<input type="hidden" id="ifsucesss" value="${ifsucesss}"/>
	<form class="layui-form"
		action="${pageContext.request.contextPath}/Manager/purchaseEditSubmit.do"
		method="post">
		<input type="hidden" name="FBillID" value="${mianList[0].FBillID}">
		<!-- ------------------头部第一行----------------------- -->
		<div class="top">
							...
							<div class="layui-input-inline">
								<select name="FW_Id" id="" class="layui-input" ">
									...
									<c:forEach items="${arealist}" var="item">
										<option value="${item.FId}"  <c:if test="${item.FId==mianList[0].FW_Id}">
										selected='selected'
										</c:if>>${item.FName}</option>
									</c:forEach>
								</select>
							</div>
						</div>
							...
							<div class="layui-input-inline">
								<select name="FJsr" id="" class="layui-input">
									<c:forEach items="${userlist}" var="item">
										<option value="${item.userid}"  <c:if test="${item.userid==mianList[0].FJsr}">
										selected='selected'
							...
							<div class="layui-input-inline">
								<select name="FS_J_Id" id="" class="layui-input">
									<c:forEach items="${driverlist}" var="item">
										<option value="${item.FId}"  <c:if test="${item.FId==mianList[0].FS_J_Id}">
										selected='selected'
										</c:if>>${item.FName}</option>
									</c:forEach>
								</select>
			...
			<table class="layui-table" style="overflow: auto; height: 400px;">
				...
				<tbody class="www">
					<c:forEach var="x" begin="0" end="9" step="1">
						<tr>
							<td>${x+1}</td>
							<td class="click"><input class="inp" type="text" disabled value="${itemsList[x].FP_Id}"></td>
							<td class="td1"><input class="inp" type="text" disabled value="${itemsList[x].category}"></td>
							<td class="td2"><input class="inp" type="text" disabled value="${itemsList[x].f_Unit}"></td>
							<td class="td3"><input name="itemsList[${x}].FP_Price"
								class="inp" type="text" value="${itemsList[x].FP_Price}" ></td>
							<td class="td4"><input name="itemsList[${x}].FP_Num"
								class="inp" type="text" value="${itemsList[x].FP_Num}"></td>
							<td class="td5"><input name="itemsList[${x}].FP_Zl"
								class="inp" type="text" value="${itemsList[x].FP_Zl}"></td>
							<td class="td6"><input name="itemsList[${x}].FP_Money"
								class="inp" type="text" value="${itemsList[x].FP_Money}"></td>
							<td class="td7"><input name="itemsList[${x}].FRemark"
								class="inp" type="text" value="${itemsList[x].FRemark}"></td>
							<td class="td8"><input name="itemsList[${x}].FP_Id"
								class="inp" type="hidden" value="${itemsList[x].FP_Id}"></td>
						</tr>
		...
		<div class="layui-row footer">
			<div class="layui-col-xs4">
				...
				<div class="layui-inline" style="margin-bottom: 5px;">
					...
					<div class="layui-input-inline">
						<input type="text" class="layui-input"  name="FSl" id="mynum"  placeholder=""  value="${mianList[0].FSl}" required>
					</div>
				</div>
				<div class="layui-inline" style="margin-bottom: 5px;">
					<label class="layui-form-label">合计金额：</label>
					<div class="layui-input-inline">
						<input type="text" class="layui-input" name="FJe" id="mymoney" placeholder=""  value="${mianList[0].FJe}" required>
					</div>
				</div>
				<div class="layui-inline" style="margin-bottom: 5px;">
